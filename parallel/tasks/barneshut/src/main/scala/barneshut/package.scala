...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    val massX: Float = if (mass == 0) centerX else quads.map(b => b.mass * b.massX).sum / mass
    val massY: Float = if (mass == 0) centerY else quads.map(b => b.mass * b.massY).sum / mass
    val total: Int = nw.total + ne.total + sw.total + se.total

    def insert(b: Body): Fork =
      if (distance(nw.centerX, nw.centerY, b.x, b.y) < nw.size) Fork(nw.insert(b), ne, sw, se)
      else if (distance(ne.centerX, ne.centerY, b.x, b.y) < ne.size) Fork(nw, ne.insert(b), sw, se)
      else if (distance(sw.centerX, sw.centerY, b.x, b.y) < sw.size) Fork(nw, ne, sw.insert(b), se)
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    def insert(b: Body): Quad =
      if (size <= minimumSize) copy(bodies = b +: bodies)
      else {
        val halfSize = size / 2
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      def position(left: Float, right: Float): Int =
        math.min(sectorPrecision - 1, math.max(((left - right) / sectorSize).toInt, 0))
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      matrix.indices foreach { index => matrix(index) = matrix(index).combine(that.matrix(index)) }
      this
    }
