...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = goal.equals(b.b1) && goal.equals(b.b2)
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.toStream.map { case (block, move) => (block, history :+ move) }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.headOption.map { case (_, moves) => moves }.getOrElse(Nil)
}
