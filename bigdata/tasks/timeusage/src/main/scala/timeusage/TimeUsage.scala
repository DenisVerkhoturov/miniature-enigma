...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = StructType(
    StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map(StructField(_, DoubleType, nullable = false))
  )
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row(line.head :: line.tail.map(_.trim.toDouble) : _*)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val leisurePrefixes = Set("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val groups = Seq(primaryPrefixes, workingPrefixes, leisurePrefixes)
    val groupedColumns = columnNames.groupBy(columnName => groups.find(_.exists(columnName.startsWith)))
                                    .mapValues(_.map(column))
                                    .withDefaultValue(Nil)
    (groupedColumns(Some(primaryPrefixes)), groupedColumns(Some(workingPrefixes)), groupedColumns(Some(leisurePrefixes)))
  }
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    val averageTimes: DataFrame = summed.groupBy('working, 'sex, 'age).agg(
      round(avg('primaryNeeds),1).as("primaryNeeds"),
      round(avg('work),1).as("work"),
      round(avg('other),1).as("other")
    )
    averageTimes.orderBy('working, 'sex, 'age)
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def scale(double: Double, scale: Int): Double = (double * scale * 10).round / (scale * 10D)

    summed
      .groupByKey(row => (row.working, row.sex, row.age))
      .agg(avg(_.primaryNeeds), avg(_.work), avg(_.other))
      .map { case ((working, sex, age), primary, work, other) =>
        TimeUsageRow(working, sex, age,  scale(primary, 1), scale(work, 1), scale(other, 1))
      }
